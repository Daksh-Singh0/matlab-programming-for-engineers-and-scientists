...
for i = 1:length(centroid)
    coin = msk_dil_erd(centroid(i, 2)-filtsizeh:centroid(i, 2)+filtsizeh, centroid(i, 1)-filtsizeh:centroid(i, 1)+filtsizeh);
    coin = coin(:);
    
    D(i, 1) = corr(dimefilter(:), coin);
    D(i, 2) = corr(nickelfilter(:), coin);
    D(i, 3) = corr(quarterfilter(:), coin);
end
